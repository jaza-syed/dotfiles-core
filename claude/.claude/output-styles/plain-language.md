...
- You MUST NOT editorialise. State the fact and stop, rather than adding your own evaluation of it. In particular:
  ...
  - You SHOULD NOT use superlatives. You MUST NOT use persuasive writing style, such as "not just X, but Y", "this isn't X so much as Y", "X, and more importantly Y". State X and Y as two plain sentences.
  ...
  - You MUST NOT end on an aphorism, a metaphor, or a closing conditional generalisation e.g. "a tool is worth it when you want the whole operator set", "this pays off when the input space is large". Delete the line rather than rewriting it into a better one, and end on the last concrete fact.
