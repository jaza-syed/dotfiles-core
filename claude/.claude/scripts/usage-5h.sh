...
# account. Reads https://api.anthropic.com/api/oauth/usage (undocumented, the same data
# /usage shows) with the OAuth token from the macOS Keychain, or from
# ~/.claude/.credentials.json on Linux. Exits 1 when usage cannot be read.
