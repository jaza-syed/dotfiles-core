...
-- Generate ghostty theme file (themes/<name>). Ghostty has no dynamic config,
-- so each palette becomes a file and the theme function relinks current-theme.
local function gen_ghostty(name, c)
  local palette = {
    c.bg, c.red, c.green, c.wood,
    c.blue, c.magenta, c.teal, c.fg,
    c.gray,
    c.bright_red or c.red,
    c.bright_green or c.green,
    c.bright_yellow or c.wood,
  ...
  local lines = {
    "# GENERATED by palettes/generate.lua -- DO NOT EDIT",
    string.format("# ghostty theme: %s", name),
    string.format("background = %s", c.bg),
    string.format("foreground = %s", c.fg),
    string.format("cursor-color = %s", c.fg),
    string.format("cursor-text = %s", c.bg),
    string.format("selection-background = %s", c.bg_vis),
    string.format("selection-foreground = %s", c.fg),
  }

  for i, hex in ipairs(palette) do
    table.insert(lines, string.format("palette = %d=%s", i - 1, hex))
  end
...
local function gen_tmux(name, c)
  ...
  if light then
    local light_lines = {
      ...
      string.format("set -g @statusline-host-bg '%s'", c.bg_const),
      string.format("set -g @statusline-host-fg '%s'", c.magenta),
    ...
    local dark_lines = {
      ...
      string.format("set -g @statusline-host-bg '%s'", c.magenta),
      string.format("set -g @statusline-host-fg '%s'", c.bg),
...
for _, name in ipairs(palettes) do
  ...
  -- 4. Generate ghostty theme
  emit("ghostty/.config/ghostty/themes/" .. name, gen_ghostty(name, palette))

  -- 5. Generate tmux colors
  emit("tmux/.config/tmux/colors-" .. name .. ".conf", gen_tmux(name, palette))

  -- 6. Generate shell fzf theme
  emit("shell/.config/shell/fzf-theme-" .. name .. ".sh", gen_fzf(name, palette))

  -- 7. Generate AeroSpace/JankyBorders mode colors
...
-- 10. Generate the nvim background registry for the OSC 11 fallback path
print("\n[registry]")
